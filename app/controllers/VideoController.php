<?php
class VideoController extends MsController{
    ...
    public function actionView(){
        //MSS::app()->accessCheck('Admin,Суперчеловек ;),SuperUser');//проверка на соответствие прав ПРОВЕРКА перенесена в модель (КАК ИСКЛЮЧЕНИЕ)
    ...
    public function actionIndex(){
        ...
        //die(MSS::$user_role);
        //ВНИМАНИЕ в виде ИСКЛЮЧЕНИЯ (права на скрытые разделы проверяются в методе AllVideoData() класса MsDBProcess)
        $GLOBALS['mss_monitor'][] = '<hr>Текущее действие: action'.$this->action.' контроллера: '.$this->model_name.'Controller<br>';
        $model = $this->loadModel();//подключаем модель
        //формируем объект с данными для использования в файле отображения 
